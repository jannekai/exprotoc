defmodule Mix.Tasks.Compile.Exprotoc do
  ...
  def run(_) do
    ...
    { :ok, proto_files } = Keyword.fetch Mix.Project.config, :proto_files
  ...
  defp get_out_dir do
    path = Keyword.fetch Mix.Project.config, :proto_out
    if path == :error do
      { :ok, cwd } = File.cwd
  ...
  defp get_path do
    path = Keyword.fetch Mix.Project.config, :proto_path
    if path == :error do
      { :ok, [] }
    else
      path
  ...
  defp get_namespace do
    namespace = Keyword.fetch Mix.Project.config, :proto_namespace
    if namespace == :error do
      { :ok, app_name } = Keyword.fetch Mix.Project.config, :app
